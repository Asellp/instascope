...
DÜZELTME (A4.3 — gerçek veri bulgusu, hesap ölçek farkı):
gerçek veriyle (real_data_pool.json, 7-8 hesap) test edilince, hesaplar
arası ORTALAMA BEĞENİ ölçeğinin ~700 kata varan farklar gösterdiği
bulundu (ör. bir hesapta ortalama 70, başka birinde 50.267). Model, bu
farkı ayırt edecek gerçek bir sinyale sahip değildi — account_growth_rate,
gerçek hesaplarda (takipçi geçmişi henüz toplanmadığı için) hep 0.0
çıkıyordu, yani SIFIR varyanslı bir özellik modele hiçbir bilgi
taşımıyordu. Sonuç: model düşük-etkileşimli hesaplarda sistematik olarak
FAZLA tahmin üretiyordu (havuzlanmış ortalamaya doğru kayıyordu).

Çözüm: account_recent_avg_likes eklendi — hesabın KENDİ önceki
gönderilerinin kronolojik ortalaması (naive_pred ile AYNI hesaplama,
shift(1) ile leakage'sız), ama artık sadece değerlendirme taban çizgisi
olarak değil, MODELE GİRDİ olarak da veriliyor. Bu, ekstra veri toplamadan
(zaten elimizdeki post geçmişinden), her hesap için gerçek, sıfır olmayan
bir ölçek sinyali sağlıyor.
...
# DÜZELTME: "account_recent_avg_likes" eklendi (bkz. yukarıdaki modül notu).
FEATURE_COLUMNS_NUMERIC = [
    "hour", "day_of_week", "hashtag_count", "caption_length",
    "account_growth_rate", "account_recent_avg_likes",
]
...
def add_naive_predictions(df: pd.DataFrame) -> pd.DataFrame:
    ...
    DÜZELTME: aynı hesaplama artık account_recent_avg_likes olarak da
    kopyalanıyor — biri (naive_pred) DEĞERLENDİRME taban çizgisi, diğeri
    (account_recent_avg_likes) MODELE GİRDİ. İkisi aynı sayı, ama farklı
    rol/isim taşımaları, "neden değerlendirme taban çizgisini modele
    besliyoruz" karışıklığını önlemek için bilinçli bir ayrım."""
    ...
    df["account_recent_avg_likes"] = df["naive_pred"]
    return df
...
def _fit_and_score(df: pd.DataFrame, model_type: str) -> tuple[BaselineReport, Pipeline, pd.DataFrame]:
    ...
    # (account_recent_avg_likes de naive_pred ile AYNI anda NaN olur, tek dropna yeterli.)
